...
`docs/advisor-bootstrap.md` is a concatenation of this file plus `docs/vision.md`, ADR-0001, and ADR-0002, intended to be fetched as a single raw URL by a fresh advisor chat. Whenever any of those source files change, `advisor-bootstrap.md` must be regenerated in the same PR or it goes stale and the advisor reads outdated context on the next bootstrap. Regeneration is currently a manual concatenation — a CI tripwire to fail PRs that touch a source file without regenerating the bootstrap is tracked as a separate issue.
